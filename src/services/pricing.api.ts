...
export async function createPricing(payload: PricingPayload): Promise<PricingResponse> {
  try {
    const response = await api.post('service-specialty-pricing/', { ...payload, tenant_id });
...
export const updatePricing = async (
  ...
  try {
    const response = await api.put(`service-specialty-pricing/${id}/`, payload);
    return response.data;
  } catch (error: any) {
    if (axios.isAxiosError(error)) {
