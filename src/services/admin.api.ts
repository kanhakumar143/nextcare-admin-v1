...
export const getServices = async (days?: string) => {
  try {
    const { data } = await api.get(
      `tenant-service/tenant/?tenant_id=${ORG_TENANT_ID}&limit=${days || 10}`
    );
    return data;
  } catch (error: any) {
...
export const getOnlyServices = async () => {
  try {
    const { data } = await api.get(
      `service-specialty-subservice/?tenant_id=${ORG_TENANT_ID}`
    );
    return data;
  } catch (error: any) {
    if (axios.isAxiosError(error)) {
